...
module.exports = {
  ...
  getAdmin: async function(req, res, next) {
    try {
      ...
      var data = users.filter(i => i.productNewOrder.order.length > 0);
  ...
  //Manager Users
  getManagerUsers: async function(req, res, next) {
    try {
      req.session.isManager = false;
      var count = 0;
      const user = await UserModel.find();
      user.filter(i => i.productNewOrder.order.length > 0);
  ...
  postUpdateUser: function(req, res, next) {
    const {_id, username, age, phone, role, email, address, created } = req.body;
  ...
  postlistOrder: function(req, res, next) {
    req.session.isManager = false;
    var {year} = req.body;
    const convertIntYear = Number(year);
    var count = 0;
    UserModel.find()
      .then(user => {
        var data = user.filter(i => i.productNewOrder.order.length > 0);
